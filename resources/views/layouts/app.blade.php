        ...
        <div class="bg-[#F1EADC] text-black min-h-screen transition-all duration-300 shadow-lg flex flex-col px-3 py-4">
            ...
            <ul class="space-y-2 flex-grow">
                ...
                <li class="relative">
                    ...
                    <ul x-show="manageOpen && open" x-collapse class="bg-gray-200 mt-2 rounded-lg shadow-md overflow-hidden space-y-2 p-2">
                        <li>
                            <a href="{{ route('categories.index') }}" class="flex items-center p-3 rounded transition-all duration-200 
                                {{ request()->routeIs('categories.index') ? 'bg-green-500 text-white' : 'hover:bg-gray-300' }}">
                                <i data-lucide="grid" class="w-6 h-6"></i>
                                <span x-show="open" x-transition.opacity class="ml-3">Manage Categories</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('admin.products') }}" class="flex items-center p-3 rounded transition-all duration-200 
                                {{ request()->routeIs('admin.products') ? 'bg-green-500 text-white' : 'hover:bg-gray-300' }}">
                                <i data-lucide="package" class="w-6 h-6"></i>
                                <span x-show="open" x-transition.opacity class="ml-3">Manage Products</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('admin.employees') }}" class="flex items-center p-3 rounded transition-all duration-200 
                                {{ request()->routeIs('admin.employees') ? 'bg-green-500 text-white' : 'hover:bg-gray-300' }}">
                                <i data-lucide="users" class="w-6 h-6"></i>
                                <span x-show="open" x-transition.opacity class="ml-3">Manage Employees</span>
                            </a>
                        </li>
                    </ul>
                </li>
